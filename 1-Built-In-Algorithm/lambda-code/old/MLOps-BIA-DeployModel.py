...
def lambda_handler(event, context):
    try:
        ...
        print("INFO:", "DEBUG", "Creating new endpoint configuration")
        ...
        if endpoint_environment == 'Dev':
            print("INFO:", "DEBUG", "Environment Input is Dev so Creating model resource from training artifact")
            create_model(jobName, trainingImage , modelArtifact)
        else:
            print("INFO:", "DEBUG", "Environment Input is not equal to Dev meaning model already exists - no need to recreate")
...
def create_endpoint_config(jobName,endpoint_config_name,config_param):
    ...
    """
    try:
        print("INFO:", "DEBUG", "Made it to create_endpoint_config")
            ...
            ProductionVariants=[
                {
                    ...
                    'ModelName': jobName,
                    #'InitialInstanceCount': "{}".format(config_param["InitialInstanceCount"]),
                    #'InitialVariantWeight': "{}".format(config_param["InitialVariantWeight"]),
                    'InitialInstanceCount': 1,
                    'InitialVariantWeight': 1,
                    'InstanceType': "{}".format(config_param["InstanceType"]),
                }
            ]
        )
...
def create_endpoint(endpoint_config_name):
    print("INFO:", "DEBUG", "Creating Endpoint")
...
def put_job_success(event):
    ...
    print("SUCCESS:", "DEPLOYMENT", "Deployment Complete")
    print(event['message'])
